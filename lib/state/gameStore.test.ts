...
describe('map mode', () => {
  beforeEach(() => {
    useGameStore.setState({ mapMode: 'browse', pendingBrand: null })
  })
  ...
  it('starts in browse, so a visitor clicking a logo reaches the advertiser', () => {
    expect(useGameStore.getState().mapMode).toBe('browse')
  })

  it('arms conquer when a brand first resolves', () => {
    useGameStore.getState().setPendingBrand(brand)
    expect(useGameStore.getState().mapMode).toBe('conquer')
  })

  it('does NOT re-arm after the user deliberately switched back to browsing', () => {
    // The claim bar re-publishes the same brand on re-render; without the transition guard that
    // would drag the user back into conquer mode every time and make the toggle feel broken.
    useGameStore.getState().setPendingBrand(brand)
    useGameStore.getState().setMapMode('browse')
    useGameStore.getState().setPendingBrand(brand)
    expect(useGameStore.getState().mapMode).toBe('browse')
  })

  it('leaves the mode alone when the brand is cleared', () => {
    useGameStore.getState().setPendingBrand(brand)
    useGameStore.getState().setPendingBrand(null)
    expect(useGameStore.getState().mapMode).toBe('conquer')
  })

  it('can be switched manually either way', () => {
    useGameStore.getState().setMapMode('conquer')
    expect(useGameStore.getState().mapMode).toBe('conquer')
    useGameStore.getState().setMapMode('browse')
    expect(useGameStore.getState().mapMode).toBe('browse')
  })
